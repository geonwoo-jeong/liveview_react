...
attributes, even when the consuming app installs `liveview_react` through the
local `file:` dependency generated by the installer. No wrapper component is
required. Phoenix remains responsible for event targeting, loading classes, and
reconnect behavior.
